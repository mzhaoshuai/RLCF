...
if os.path.exists(DOWNLOAD_ROOT_v1):
    ...
    raise NotImplementedError("You shoud put an available CLIP download folder (/YOUR/PATH) here")
...
def main():
    ...
    image_ids = candidates.keys()
    image_paths = [os.path.join(args.image_dir, path) for path in image_ids]
            ...
            for k, v in other_metrics.items():
                if k == 'bleu':
                    for bidx, sc in enumerate(v):
                        print('BLEU-{}: {:.2f}'.format(bidx+1, sc * 100))
                else:
                    print('{}: {:.2f}'.format(k.upper(), v * 100))
        print('CLIPScore: {:.2f}'.format(np.mean([s['CLIPScore'] for s in scores.values()]) * 100))
        print('RefCLIPScore: {:.2f}'.format(np.mean([s['RefCLIPScore'] for s in scores.values()]) * 100))
